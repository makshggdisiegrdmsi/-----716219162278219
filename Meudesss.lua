local Libary = loadstring(game:HttpGet("https://raw.githubusercontent.com/makshggdisiegrdmsi/-----716219162278219/main/magnusuilib%20(1).txt"))()

workspace.FallenPartsDestroyHeight = -math.huge

local Window = Libary:MakeWindow({
    Title = "Nick hub | BrookhavenRP ",
    SubTitle = "by:  Lynix,Nick Hub,Hogakarei,shelby,dono do drip",
    LoadText = "Carregando Nick Hub",
    Flags = "NickHub_Broookhaven"
})
 Window:AddMinimizeButton({
Button = { Image = "rbxassetid://73347901030535", BackgroundTransparency = 1 },
...
--------------------------------------------------------------------------------------------------------------------------------
                                                   -- === Tab 1: server=== --
---------------------------------------------------------------------------------------------------------------------------------
Tab1:AddSection({"Servidor do Nick hub"})

Tab1:AddDiscordInvite({
    Name = "Nick Hub | Hallowen ",
    Description = "https://discord.gg/kETed7dhc",
    Logo = "rbxassetid://88788745293038",
    Invite = "https://discord.gg/qaw9TfxFY2",
})
...
local Section = Tab1:AddSection({"Versão do Hub 1.4"})

local Paragraph = Tab1:AddParagraph({"Criadores", "Nick,Lynix,shelby,horakage"})

  
  Tab1:AddButton({
    Name = "TikTok",
    Callback = function()
      setclipboard("@Nickzinpcc") -- Copia o @
      setclipboard("https://www.tiktok.com/@nickzinpcc?_t=ZM-90k1TLdznRM&_r=1") -- Copia o link também, se quiser só o @, remova esta linha
      
    end
  })
 
 
 Tab1:AddSection({ "Rejoin" })
...
game:GetService("StarterGui"):SetCore("SendNotification", { 
    ...
    Text = "Nick Hub - Use WASD para voar!";
...
createMusicDropdown("Phonk", {
    ["phonk"] = {
        ...
        {name = "montagem luna", id = "97279560249940"},
        {name = "passo bem solto", id = "137466814003263"},
        {name = "masha ultraphonk", id = "95248559316142"},
        {name = "67 phonk", id = "122854357582130"},
        {name = "", id = ""},
        {name = "", id = ""},
        {name = "", id = ""},
        {name = "", id = ""},
...
Tab9:AddButton({"Fling Bola ALL V2", function()
    ...
    if not equipBola() then return end
    task.wait(0.5)
    local args = { [1] =o "PlayerWantsToDeleteTool", [2] = "SoccerBall" }
    pcall(function()
        ReplicatedStorage:WaitForChild("RE"):WaitForChild("1Clea1rTool1s"):FireServer(unpack(args))
    end)
    local workspaceCom = Workspace:FindFirstChild("WorkspaceCom")
...
local function ApplyOutfit(shirtId)
	if not shirtId then return end
	pcall(function()
		WearShirtRemote:InvokeServer(shirtId)
	end)
	ApplyAccessories()
	ApplyBody()
	PlayAnimation()
...
Tab9:AddDropdown({
	Name = "Selecione um Skybox",
	Options = { "Sorriso Horror", "Horror", "C00lkid" },
	Default = nil,
	Callback = function(selected)
		if selected == "Sorriso Horror" then
			SkyboxID = 8453056214
		elseif selected == "Horror" then
			SkyboxID = 231796017
		elseif selected == "C00lkid" then
			SkyboxID = 18554114295
		end
	end
})
...
Tab9:AddTextbox({
	Name = "Coloque Seu id de camisa",
	Default = "",
	PlaceholderText = "Ex: 123456789",
	Callback = function(text)
		if tonumber(text) then
			SkyboxID = tonumber(text)
		end
	end
})

-- Botão aplicar Skybox
Tab9:AddButton({"Colocar SKYBOX", function()
	if SkyboxID then
		ApplyOutfit(SkyboxID)
		showNotification("Skybox FE", "Skybox aplicado com sucesso!")
	else
		showNotification("Skybox FE", "Defina um ID primeiro!")
	end
end})

-- Botão remover Skybox
Tab9:AddButton({"Remover SKYBOX", function()
	StopAnimation()
	showNotification("Skybox FE", "Skybox removido (animação parada).")
end})
...
Tab:AddButton({
    ...
    Description = "",
    Callback = function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/267266273ffsfs/antbug/refs/heads/main/Main.txt"))()
    end
})
...
for name, position in pairs(locations) do
    Tab11:AddButton({
        Name = name,
        Description = "Teleporta até " .. name,
        Callback = function()
...
print("Nick Hub Brookhaven carregado com sucesso!")